...
- 管理侧配置数据模型：支撑默认 Agent 的模型、数据源、指标口径、知识、语义、Skill、预设问题和访问 API 配置。
- 用户侧运行数据模型：支撑会话管理、会话展示、文本问数、请求幂等和问数结果展示。语音问数作为后续专项预留，不进入一期问答主链路。
...
一期仅保留语音入口状态和降级提示，不进入问答主链路。语音识别模型、语音识别记录和音频留存属于 P2 专项能力，不能阻塞文本问数 MVP 上线。

### 2.5 指标口径结构化发布

指标是否可查、采用哪个版本、是否默认版本，必须以结构化指标口径定义为准。知识库、RAG 和业务术语只负责解释文本、同义词和背景说明，不作为口径发布状态的权威来源。
...
模型资源配置用于维护默认 Agent 依赖的大语言模型和嵌入模型，并预留语音识别模型分类。
...
| modelCategory | 模型分类 | 枚举 | 是 | LLM、EMBEDDING；ASR 为 P2 预留 |
...
| toolName | 后端工具名 | 字符串 | 是 | 智能体实际调用的固定工具名 |
...
- `skillCode` 用于配置和展示，`toolName` 用于智能体工具调用。
- 一期 `toolName` 固定为 `workOrderStatusSummary`、`inventoryBelowSafetyStock`、`equipmentDowntimeSummary`。

### 4.6 指标口径定义

指标口径定义用于维护正式可查指标的版本、生效状态、默认版本和工具绑定。

| 字段编码 | 字段名称 | 类型 | 必填 | 说明 |
|---|---|---|---|---|
| metricId | 指标口径标识 | 字符串 | 是 | 指标口径唯一标识 |
| agentId | Agent 标识 | 字符串 | 是 | 所属默认 Agent |
| metricKey | 指标编码 | 字符串 | 是 | 稳定业务编码 |
| metricName | 指标名称 | 字符串 | 是 | 页面和审计展示名称 |
| domain | 业务域 | 字符串 | 是 | 工单、库存、设备等 |
| version | 版本号 | 字符串 | 是 | 口径版本 |
| status | 发布状态 | 枚举 | 是 | 草稿、已发布、停用 |
| defaultFlag | 默认版本 | 布尔 | 是 | 同一指标仅允许一个已发布默认版本 |
| toolName | 后端工具名 | 字符串 | 是 | 允许调用的工具 |
| calculationRule | 计算规则 | 长文本/CLOB | 是 | 指标计算规则 |
| effectiveTime | 生效时间 | 日期时间 | 否 | 指标口径生效时间 |
| publishedBy | 发布人 | 字符串 | 否 | 发布操作人 |
| publishedTime | 发布时间 | 日期时间 | 否 | 发布时间 |
| updatedTime | 更新时间 | 日期时间 | 否 | 最近更新时间 |

说明：

- 指标口径定义是 CALL_TOOL 前的权威校验来源。
- 只有 `status=PUBLISHED` 的指标允许进入正式工具调用。
- RAG 和知识库不得决定指标是否已发布。
...
| agentId | Agent 标识 | 字符串 | 是 | 固定关联默认 Agent |
| clientMessageId | 前端幂等消息 ID | 字符串 | 是 | 问答主链路必填 |
| messageRole | 消息角色 | 枚举 | 是 | 用户、Agent、系统 |
| inputType | 输入类型 | 枚举 | 是 | 文本、预设问题、系统；语音为 P2 预留 |
...
- 会话消息只保存最终用户消息和 Agent 回复，不承担工具调用前的幂等状态机职责。
- 同一 `sessionId + clientMessageId` 的执行状态必须先写入问答请求台账。

### 5.3 问答请求台账

问答请求台账用于在调用 LLM、RAG 和 MOM 工具前记录请求状态，防止同一 `sessionId + clientMessageId` 重复执行工具。

| 字段编码 | 字段名称 | 类型 | 必填 | 说明 |
|---|---|---|---|---|
| ledgerId | 台账标识 | 字符串 | 是 | 请求台账唯一标识 |
| sessionId | 会话标识 | 字符串 | 是 | 所属会话 |
| agentId | Agent 标识 | 字符串 | 是 | 固定关联默认 Agent |
| clientMessageId | 前端幂等消息 ID | 字符串 | 是 | 与客户端提交一一对应 |
| traceId | 调用链标识 | 字符串 | 是 | 串联请求、消息、结果和审计 |
| inputMode | 输入模式 | 枚举 | 是 | text、preset；speech 为 P2 预留 |
| questionDigest | 问题摘要 | 字符串 | 是 | 用户问题哈希或摘要 |
| questionSnapshotRef | 问题快照引用 | 字符串 | 否 | 原文快照引用，需满足脱敏和留存策略 |
| requestStatus | 请求状态 | 枚举 | 是 | RECEIVED、PROCESSING、SUCCEEDED、PERSIST_FAILED、FAILED |
| lastStage | 最近阶段 | 字符串 | 是 | RECEIVED、INTENT、CALL_TOOL、SAVE_MESSAGES 等 |
| toolName | 工具名 | 字符串 | 否 | 已选择的工具 |
| toolResultDigest | 工具结果摘要 | 字符串 | 否 | 工具结果哈希或摘要 |
| answerSnapshot | 回答快照 | 长文本/CLOB | 否 | 已生成回答，用于重试恢复 |
| attemptCount | 尝试次数 | 整数 | 是 | 请求处理次数 |
| heartbeatTime | 心跳时间 | 日期时间 | 否 | 处理中最近心跳 |
| lockedUntil | 锁定截止时间 | 日期时间 | 否 | 防止并发重复处理 |
| errorCode | 错误码 | 字符串 | 否 | 失败原因编码 |
| errorMessage | 错误说明 | 字符串 | 否 | 失败原因 |
| createdTime | 创建时间 | 日期时间 | 否 | 创建时间 |
| updatedTime | 更新时间 | 日期时间 | 否 | 最近更新时间 |

规则：

- 新请求必须先创建台账，再进入 LLM、RAG 或工具调用。
- `sessionId + clientMessageId` 必须全局唯一。
- 工具调用前必须写入 `lastStage=CALL_TOOL`、`heartbeatTime` 和 `lockedUntil`。
- `PERSIST_FAILED` 表示工具已调用但消息或结果落库失败，重试时不得再次调用工具。

### 5.4 语音识别记录

语音识别记录用于后续 P2 语音专项，记录语音输入识别后的文本和识别状态。一期不进入问答主链路。
...
- 无数据是业务成功空结果，不是错误。
- `resultStatus=NO_DATA` 时 `errorCode` 必须为空，历史会话按完成态处理。
...
- 一期问答主链路仅允许 TEXT 和 PRESET_QUESTION。
- VOICE 仅作为 P2 预留枚举，不能触发一期问答工具调用。
