...
MOM 智能问数 MVP 需要先跑通一个默认 Agent 的配置、访问和问数闭环。管理侧用于内部管理员维护默认 Agent 所需的模型、MOM 数据源、知识、语义、Skill、预设问题和语音输入配置，使用户侧能够基于已配置能力调用默认 Agent 完成自然语言问数。系统提示词在 MVP 阶段内置，不对用户开放修改。
...
| 目标 | 说明 |
|---|---|
| 支持默认 Agent 配置 | MVP 阶段只支持一个 Agent，所有管理侧配置均归属于该 Agent。 |
| 支持基础资源配置 | 管理 LLM、嵌入模型和 MOM 业务数据源。 |
| 支持 Agent 能力配置 | 管理智能体知识、业务知识、语义模型和 Skill。 |
| 支持问数使用配置 | 管理用户侧问答展示的预设问题。 |
| 支持语音输入配置 | 提供后端 ASR 代理所需配置，前端上传 WAV 后由后端完成识别。 |
| 支持配置测试 | 管理员能够测试模型连接、数据源连接、知识召回、Skill 可用性等关键配置。 |
| 支持 MVP 问数闭环 | 配置完成后，用户侧可通过文本输入或语音识别后的文本调用默认 Agent 完成基础问数。 |

### 1.3 适用角色

| 角色 | 使用目标 |
|---|---|
| 内部管理员 | 维护模型、MOM 数据源、知识、语义模型、Skill、预设问题和语音输入配置。 |
...
- 外部 API 调用入口、API Key 生成/重置/删除/启停和访问 API 页面开放
- 原始音频持久化、独立语音识别记录和语音识别后自动问数
- ERP、APS 等其他产品库问数、多数据源路由、跨数据源 Join 和跨系统数据集成
...
MVP 阶段采用 **“一个默认 Agent + 管理侧配置 + 用户侧问答”** 的方式。管理员在管理侧完成默认 Agent 的基础配置和语音输入配置，用户侧负责发起文本问数，或通过后端 ASR 代理识别语音后确认发送文本并查看结果。
...
- 访问 API、外部 API 和 API Key 管理在 MVP 阶段完全隐藏，不进入开发和联调。
- 默认 Agent 仅绑定一个生效 MOM 数据源，不接 ERP、APS 等其他产品库。
- 阻断项配置不完整时，用户侧 Chat 不应进入正式问数状态；非阻断项配置缺失时可提示风险或隐藏对应入口。

默认 Agent 可用性口径：

| 配置项 | 是否阻断正式问数 | 说明 |
|---|---|---|
| LLM 未配置或不可用 | 是 | 默认 Agent 无法生成问数结果。 |
| 嵌入模型未配置或不可用 | 是 | 影响知识向量化和召回能力，MVP 阶段作为基础依赖处理。 |
| 生效数据源不存在 | 是 | 无法访问业务数据。 |
| 数据源连接测试失败 | 是 | 不要求在此处判断是否为 MOM 数据源，只判断当前生效数据源连接是否可用。 |
| Schema 未初始化或初始化失败 | 是 | 无法稳定识别表字段和结构。 |
| 语义模型未就绪 | 否 | 可继续问数，但结果准确性和指标口径一致性存在风险。 |
| Skill 无可用配置 | 否 | 不阻断基础问数。 |
| 知识配置为空或未向量化 | 否 | 不阻断基础问数，但知识增强效果受影响。 |
| 预设问题为空 | 否 | 不影响用户手动输入。 |
| 语音配置停用或未配置 | 否 | 不影响文本问数；用户点击语音识别时提示语音输入不可用。 |
...
| 范围 | 说明 |
|---|---|
| 模型配置 | 配置 LLM、Embedding 模型，并支持测试。 |
| 数据源配置 | 配置 MOM 业务数据库连接，支持连接测试和 Schema 初始化。 |
| 智能体知识配置 | 管理增强默认 Agent 能力的文档、问答对和 FAQ。 |
| 业务知识配置 | 管理业务术语、状态枚举、业务规则和不可回答边界。 |
| 语义模型配置 | 管理业务对象、字段别名、表关系、指标口径和枚举映射。 |
| Skill 管理 | 支持 ZIP 格式 Skill 导入、删除、启用、停用和测试。 |
| 预设问题管理 | 管理用户侧 Chat 首页或输入区展示的预设问题（用户侧展示为推荐问题）。 |
| 语音输入配置 | 提供后端 ASR 代理所需配置，前端上传 WAV，后端调用 ASR 并返回识别文本。 |
| 问数闭环 | 支持用户侧通过文本输入或语音识别后的文本调用默认 Agent 完成问数。 |
...
| 模型配置可用 | LLM、嵌入模型至少完成必填项保存，并能展示测试状态。 |
| 数据源可用 | MOM 数据源连接测试成功，Schema 初始化成功，并可展示表和字段初始化结果。 |
| 业务知识可用 | 至少支持维护业务术语、状态枚举或业务规则。 |
| 语义模型可用 | 至少支持维护业务对象、字段别名、表关系和指标口径中的基础配置。 |
| Skill 可用 | 至少支持 ZIP 导入、自动安装、列表查看、启用/停用、删除和测试入口。 |
| 预设问题可用 | 用户侧 Chat 能展示已启用的预设问题。 |
| 语音输入配置可用 | 前端可读取语音入口配置，上传 WAV 后由后端 ASR 代理返回识别文本并填入输入框。 |
| 文本问数闭环可用 | 用户侧输入一个基础问数问题后，默认 Agent 能返回结果或明确边界提示。 |
| 语音输入问数可用 | 用户侧语音识别后，用户确认发送文本，默认 Agent 能返回结果或明确边界提示。 |
...
- “访问 API”为后置能力，MVP 阶段完全隐藏，不作为管理侧菜单入口。

管理菜单原型图（仅供功能参考，不代表最终效果）：
![image-20260608142309361](../assets/requirements/管理菜单.png)

#### 2.3.2 模型配置

| 项目 | 说明 |
|---|---|
| 功能目标 | 配置默认 Agent 使用的 LLM 和嵌入模型 |
| 适用角色 | 管理员 |
| 前置条件 | 默认 Agent 已存在 |
| 触发方式 | 管理侧菜单进入“模型配置” |
...
![image-20260615211406250](../assets/requirements/模型配置.png)
...
| 项目 | 说明 |
|---|---|
| 功能目标 | 配置默认 Agent 可访问的 MOM 数据源，支持连接测试、Schema 初始化和核心表范围选择 |
| 适用角色 | 管理员 |
| 前置条件 | 默认 Agent 已存在 |
| 触发方式 | 管理侧菜单进入“数据源配置” |
...
![image-20260615211420871](../assets/requirements/数据源配置.png)
...
| 分组 | 字段 |
|---|---|
| 基础信息 | 数据源名称、数据源类型（MVP 当前仅支持 MySQL，其他数据库类型后置）、连接状态、启用状态 |
...
- 管理员可选择核心表或排除表。
- MVP 阶段默认 Agent 仅允许一个生效 MOM 数据源参与问数。
...
| 项目         | 说明                                                       |
| ------------ | ---------------------------------------------------------- |
| 功能目标     | 管理默认 Agent 可调用的 Skill 能力，支持 ZIP 导入、删除、启停和测试 |
| 适用角色     | 管理员                                                     |
| 前置条件     | 默认 Agent 已存在                                          |
| 触发方式     | 管理侧菜单进入“Skill 管理”                                 |
| 系统处理逻辑 | 管理员上传 ZIP 格式 Skill 包，系统完成校验和自动安装；管理员可删除、启用、停用并进行基础测试 |
| 输出结果     | 保存 Skill 元数据、安装状态、存储路径和启停状态，默认 Agent 仅能调用已启用且安装成功的 Skill |
| 异常处理     | 文件格式不支持、超过 100MB、安装失败或测试失败时展示失败原因；未启用 Skill 不可被默认 Agent 调用 |
| 涉及页面     | Skill 管理页                                               |

验收标准：

- 管理员可查看 Skill 列表。
- 管理员可导入 ZIP 格式 Skill 包，单个包大小不超过 100MB。
- 上传成功后系统自动安装，安装失败需展示失败原因。
- 管理员可启用或停用 Skill。
- 管理员可删除 Skill。
- 已停用 Skill 不应被默认 Agent 调用。

Skill管理原型图（仅供功能参考，不代表最终效果）：
![image-20260608162707320](../assets/requirements/Skill管理.png)
...
![image-20260610134328952](../assets/requirements/智能体知识配置.png)
...
![image-20260610134417294](../assets/requirements/业务知识.png)
...
![image-20260610134439537](../assets/requirements/语义模型.png)
...
![image-20260610134510685](../assets/requirements/预设问题管理.png)

#### 2.3.10 语音输入配置

本功能用于提供用户侧语音输入所需的后端 ASR 代理配置。MVP 阶段语音仅作为输入辅助，前端采集音频并转换为 WAV 后上传后端，后端调用 ASR 服务并返回识别文本，前端将文本填入输入框，用户确认后按普通文本问数发送；后端不持久化原始音频，不保存独立语音识别记录。

| 项目 | 说明 |
|---|---|
| 功能目标 | 维护后端 ASR 代理所需配置，支撑用户侧语音输入 |
| 适用角色 | 管理员 |
| 前置条件 | 默认 Agent 已存在 |
| 触发方式 | 管理侧菜单进入“语音输入配置” |
| 系统处理逻辑 | 管理员维护语音识别启用状态、供应方配置和提示文案；用户侧读取非敏感配置后上传 WAV，后端代理调用 ASR 并返回识别文本 |
| 输出结果 | 保存语音输入配置，用户侧可按配置展示语音入口和识别提示 |
| 异常处理 | 配置缺失或停用时，用户侧隐藏语音入口或提示语音输入不可用；文本问数不受影响 |
| 涉及页面 | 语音输入配置页、用户侧问答页面 |
...
| 语音入口 | 是否启用、识别方式、支持语言、支持格式、文件大小限制、录音时长限制、ASR 超时时间、提示文案 | 控制用户侧是否展示语音输入入口和上传约束。 |
| 供应方配置 | 供应商、服务地址、访问密钥、模型名称或识别参数 | 供后端 ASR 代理调用使用，敏感字段保存后脱敏展示，不下发前端。 |
| 前端提示 | 录音提示、识别中提示、识别失败提示 | 用于用户侧交互文案。 |

处理规则：

1. MVP 阶段语音输入只负责把语音转换成文本并填入输入框。
2. 用户确认识别文本后手动发送，后端按普通文本问数处理。
3. 前端只上传 WAV 音频；后端代理调用 ASR 服务，不持久化原始音频，不保存独立语音识别记录。
4. 语音输入配置不影响文本问数；配置不可用时文本问数仍可继续使用。
5. 供应方访问密钥只在后端保存和使用，查询时默认脱敏展示，不向前端下发。
6. 后端需校验文件大小、音频时长、MIME 类型和 WAV 文件头；格式不合法、文件超限或时长超限时不调用 ASR 供应方。
7. 后端需设置 ASR 调用超时和调用频率限制；超时、供应方失败、识别为空或调用频繁时返回明确错误，不进入问数链路。

验收标准：

- 管理员可保存语音输入配置。
- 用户侧可读取语音输入配置并展示或隐藏语音入口。
- 前端上传 WAV 后，后端 ASR 代理返回识别文本并填入输入框，不自动提交。
- 管理员可维护语音支持格式、最大文件大小、最大录音时长和 ASR 超时时间。
- 用户手动发送后，系统按文本问数链路返回结果或边界提示。
...
| 数据模型文档 | 与本需求关系 |
|---|---|
| MOM智能问数-MVP数据模型文档 | 作为本需求的数据对象定义依据，明确默认 Agent、模型资源、MOM 数据源、Skill、知识、语义字段映射、预设问题、语音输入配置、会话、消息和结果 Payload 等对象的字段口径、对象关系和状态规则。 |

#### 2.4.2 关键配置对象

| 模型 | 属性 | 属性名称 | 与本需求关系 | 说明 |
|---|---|---|---|---|
| 默认 Agent | agentId | Agent 标识 | 配置归属 | MVP 阶段仅存在一个默认 Agent，所有管理配置均归属于该对象。 |
| 默认 Agent | agentName | Agent 名称 | 页面展示 | 管理侧顶部和配置归属展示使用。 |
| 模型资源配置 | modelCategory | 模型分类 | 功能配置 | 区分大语言模型、嵌入模型。 |
| 模型资源配置 | baseUrl | 服务地址 | 功能配置 | 用于模型服务调用。 |
| 模型资源配置 | apiKey | 访问密钥 | 敏感配置 | 页面需脱敏展示。 |
| 模型资源配置 | modelName | 模型名称 | 功能配置 | 标识具体可调用模型。 |
| 数据源配置 | datasourceType | 数据源类型 | 功能配置 | MVP 当前固定为 MySQL；Oracle、SQL Server、PostgreSQL 等数据库类型后置。 |
...
| Skill 配置 | skillCode | Skill 编码 | 能力白名单 | 标识默认 Agent 可调用的 Skill。 |
| Skill 配置 | enabledFlag | 启用状态 | 能力白名单 | 控制 Skill 是否可调用。 |
| Skill 配置 | packagePath | ZIP 包存储路径 | Skill 管理 | 标识上传原包的存储位置。 |
| Skill 配置 | installPath | 安装目录 | Skill 管理 | 标识自动安装后的文件目录。 |
| Skill 配置 | installStatus | 安装状态 | Skill 管理 | 标识 Skill 是否安装成功。 |
...
| 语音输入配置 | voiceEnabledFlag | 语音入口启用状态 | 用户侧输入辅助 | 控制用户侧是否展示语音输入入口。 |
| 语音输入配置 | provider | 供应商 | 用户侧输入辅助 | 标识后端 ASR 代理调用的服务或能力。 |
| 语音输入配置 | apiKey | 访问密钥 | 敏感配置 | 页面需脱敏展示，只供后端 ASR 代理使用。 |
| 语音输入配置 | maxFileSizeMb | 最大文件大小 | 用户侧输入辅助 | 限制单次上传音频大小，默认 10MB。 |
| 语音输入配置 | maxDurationSeconds | 最大录音时长 | 用户侧输入辅助 | 限制单次语音输入时长，默认 60 秒。 |
| 语音输入配置 | allowedMimeTypes | 允许 MIME 类型 | 用户侧输入辅助 | MVP 仅允许 `audio/wav`、`audio/x-wav`。 |
| 语音输入配置 | asrTimeoutSeconds | ASR 调用超时 | 用户侧输入辅助 | 限制后端调用 ASR 服务的等待时间，默认 30 秒。 |
| 系统提示词 | promptContent | 系统提示词内容 | 内置能力 | 由系统内置，用于约束默认 Agent 行为，不对用户开放修改。 |
...
| Skill 配置 | enabledFlag | 启用状态 | 控制默认 Agent 是否可调用 | Skill 管理页 | 启用、停用。 |
| Skill 配置 | installStatus | 安装状态 | 标识 Skill 包安装结果 | Skill 管理页 | 未安装、安装中、成功、失败。 |
...
| 语音输入配置 | voiceEnabledFlag | 语音入口启用状态 | 控制用户侧是否展示语音入口 | 语音输入配置页 | 启用、停用。 |

#### 2.4.4 配置数据类别

| 类别 | 名称 | 载体 | 用途 | 是否必需 | 说明 |
|---|---|---|---|---|---|
| 基础资源 | 模型资源配置 | 管理侧配置表 | 支撑默认 Agent 调用模型 | 是 | 至少配置大语言模型和嵌入模型。 |
| 基础资源 | 数据源配置 | 管理侧配置表 | 支撑默认 Agent 访问 MOM 业务数据 | 是 | MVP 阶段仅一个生效 MOM 数据源参与问数。 |
| 基础资源 | 数据表范围配置 | 管理侧配置表 | 控制哪些表进入问数范围 | 是 | 与数据源初始化和语义配置联动。 |
| Agent 能力 | 系统提示词 | 系统内置配置 | 约束默认 Agent 行为 | 是 | 系统内置，不对用户开放修改。 |
| Agent 能力 | Skill 配置 | 管理侧配置表 | 控制默认 Agent 可调用能力 | 是 | 本期纳入管理范围。 |
| Agent 能力 | 智能体知识条目 | 知识库 / 向量索引 | 增强默认 Agent 知识召回能力 | 是 | 支持文档、问答对、FAQ。 |
...
| 输入辅助 | 语音输入配置 | 配置表 | 支撑用户侧语音识别输入 | 是 | 前端上传 WAV，后端 ASR 代理调用并返回识别文本。 |
...
| 已启用 | Skill、语义字段映射、预设问题、语音输入配置等 | 当前处于可用状态，参与问数、展示或输入辅助。 |
| 已停用 | Skill、语义字段映射、预设问题、语音输入配置等 | 当前不参与问数、用户侧展示或输入辅助。 |
| 未测试 | 模型资源、数据源 | 尚未进行测试。 |
...
| 安装成功 | Skill | Skill 包已完成自动安装，可在启用后被默认 Agent 调用。 |
| 安装失败 | Skill | Skill 包安装失败，需要展示失败原因。 |
...
| Skill 文件格式不支持或超过大小限制 | 禁止导入并提示仅支持 ZIP 且单包不超过 100MB。 |
| Skill 安装失败 | 保留失败记录和失败原因，Skill 不允许启用调用。 |
| Skill 测试失败 | 展示失败原因，不影响启停状态。 |
| 语音输入配置不可用 | 用户侧隐藏语音入口或提示语音输入不可用，文本问数仍可继续使用。 |
...
| 调用关系 | 用途 | 输入 | 输出 | 失败处理 |
|---|---|---|---|---|
| 管理侧保存模型配置 | 保存 LLM、嵌入模型配置 | 模型配置表单 | 保存结果、配置状态 | 返回失败原因，页面保留填写内容。 |
...
| 管理侧导入 Skill | 上传并自动安装 Skill 包 | ZIP 文件、Skill 基本信息 | Skill 元数据、安装状态、失败原因 | 格式错误、超过 100MB 或安装失败时返回失败原因。 |
| 管理侧测试 Skill | 验证 Skill 是否可调用 | Skill 标识、测试参数 | 测试结果、失败原因 | 状态更新为测试失败。 |
| 管理侧保存语音输入配置 | 保存后端 ASR 代理配置 | 语音输入配置表单 | 保存结果、启用状态 | 返回失败原因，页面保留填写内容。 |
...
| 影响范围 | 说明 |
|---|---|
| 管理侧菜单 | 需新增基础资源配置、Agent 能力配置、测试与使用配置相关菜单；访问 API 在 MVP 阶段隐藏。 |
| 用户侧 Chat | 需读取预设问题，并调用默认 Agent 完成问数。 |
| 默认 Agent 运行链路 | 需读取模型、数据源、系统提示词、知识、语义模型和 Skill 配置。 |
| 配置数据 | 需新增模型、MOM 数据源、知识、业务知识、语义模型、Skill、预设问题、语音输入配置等配置对象；系统提示词以内置方式提供。 |
| 安全控制 | 模型 API Key、语音供应方访问密钥、数据库密码需脱敏展示；数据源建议只读；危险 SQL 拦截配置需生效。 |
| 测试流程 | 每个配置模块需支持基础测试或校验，支撑 MVP 验收。 |
| 后续扩展 | 数据结构需保留默认 Agent 归属字段，便于后续扩展多 Agent，但本阶段不开放多 Agent 功能。 |
...
| Skill 导入 | 上传 ZIP 格式 Skill 包。 | 系统自动安装并展示安装状态。 |
| Skill 启用 | 启用一个安装成功的 Skill。 | 默认 Agent 可调用该 Skill。 |
| 预设问题展示 | 新增并启用预设问题。 | 用户侧 Chat 展示该问题。 |
| 语音输入配置 | 保存语音输入配置。 | 用户侧可读取配置并展示语音入口。 |
| 默认 Agent 问数 | 用户侧输入基础问数问题。 | 返回问数结果或明确边界提示。 |
| 语音输入问数 | 用户侧上传 WAV 后由后端 ASR 代理返回识别文本，用户手动发送。 | 系统按文本问数返回结果或明确边界提示。 |
...
| Skill 文件超过大小限制 | 上传超过 100MB 的 ZIP 包。 | 禁止导入并提示大小限制。 |
| Skill 停用后调用 | 默认 Agent 尝试调用停用 Skill。 | 不允许调用该 Skill。 |
| 语音输入配置停用 | 用户侧进入问数页面。 | 不展示语音入口或提示语音输入不可用。 |
...
| 访问 API 隐藏 | 管理侧查找访问 API 页面或 API Key 管理入口。 | MVP 阶段不提供页面入口和 API Key 管理能力。 |
